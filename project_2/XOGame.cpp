	...
	while (true)
	{
		...
		if (RoundEnded)
		{
			...
			if (cin.fail() || PlayAgain != 1) {
				...
				std::copy(std::begin(DefaulArray[0]), std::end(DefaulArray[0]), std::begin(ArrayPrint[0]));
				std::copy(std::begin(DefaulArray[1]), std::end(DefaulArray[1]), std::begin(ArrayPrint[1]));
				std::copy(std::begin(DefaulArray[2]), std::end(DefaulArray[2]), std::begin(ArrayPrint[2]));
				continue;
			}
		}
